...
//		}
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					deleteMyServer(tobeDeleteUser.getUsername());
					// 删除db和内存中此用户的数据
					...
					getActivity().runOnUiThread(new Runnable() {
						public void run() {
							pd.dismiss();
							UIHelper.ToastMessage(getActivity(), st2 + e.getMessage());
						}
					});
	 ...
	 * @param fid 
	 */
	private void deleteMyServer(String fid){
		RequestParams params=TXApplication.getParams();
		params.addBodyParameter("uid", TXApplication.getUserMess().getUid());
		params.addBodyParameter("fid", fid);
		String url=URLs.SPACE_DELETEFRIEND;
		TXApplication.post(null, getActivity(), url, params, new RequestCallBack<String>() {

			@Override
			public void onFailure(HttpException error, String msg) {
				UIHelper.ToastMessage(getActivity(), R.string.net_work_error);
			}
