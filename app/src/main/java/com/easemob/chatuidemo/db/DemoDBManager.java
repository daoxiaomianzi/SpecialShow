        ...
        if (db.isOpen()) {
            ...
            for (User user : contactList) {
                ...
                if(user.getIcon() != null)
                	values.put(UserDao.COLUMN_NAME_ICON, user.getIcon());
    ...
    synchronized public Map<String, User> getContactList() {
        ...
        if (db.isOpen()) {
            ...
            while (cursor.moveToNext()) {
                ...
                String icon = cursor.getString(cursor.getColumnIndex(UserDao.COLUMN_NAME_ICON));
                User user = new User();
		...
		if (db.isOpen()) {
			...
			for (RobotUser item : robotList) {
				...
				if (item.getIcon()!= null)
					values.put(UserDao.ROBOT_COLUMN_NAME_ICON, item.getIcon());
	...
	synchronized public Map<String, RobotUser> getRobotList() {
		...
		if (db.isOpen()) {
			...
			while (cursor.moveToNext()) {
				...
				String icon = cursor.getString(cursor.getColumnIndex(UserDao.ROBOT_COLUMN_NAME_ICON));
