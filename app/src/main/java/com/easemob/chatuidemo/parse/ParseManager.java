...
public class ParseManager {
			...
			public void done(List<ParseObject> arg0, ParseException arg1) {
				if (arg0 != null) {
					...
					for (ParseObject pObject : arg0) {
						...
						ParseFile parseFile1 = pObject.getParseFile(CONFIG_Icon);
						if (parseFile1 != null) {
							user.setAvatar(parseFile1.getUrl());
						}
		...
		pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
			...
			public void done(ParseObject pUser, ParseException e) {
				if(pUser!=null){
					...
					ParseFile pFile1 = pUser.getParseFile(CONFIG_Icon);
					if(callback!=null){
						...
						if(user!=null){
							user.setNick(nick);
							if (pFile1 != null && pFile1.getUrl() != null) {
								user.setIcon(pFile1.getUrl());
							}
						}
	...
	public String uploadParseAvatar(byte[] data) {
		...
		try {
			...
			ParseFile pFile1 = new ParseFile(data);
			pUser.put(CONFIG_Icon, pFile1);
			pUser.save();
			return pFile.getUrl();
		} catch (ParseException e) {
			if (e.getCode() == ParseException.OBJECT_NOT_FOUND) {
