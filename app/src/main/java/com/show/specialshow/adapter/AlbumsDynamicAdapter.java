...
import com.show.specialshow.activity.AlbumsDynamicActivity;
...
public class AlbumsDynamicAdapter extends BaseAdapter {
    ...
    private AlbumsDynamicActivity mContext;
    OnItemViewClickListener listener;

    public AlbumsDynamicAdapter(AlbumsDynamicActivity context, List<CircleDynamicItem> list) {
    ...
    private void bindData(int type, ViewHolder... holders) {
        ...
        if (StringUtils.isEmpty(item.getStatus_content())) {
            ...
            holders[type].relation_me.setText(SmileUtils.getSmiledText(mContext,
                    item.getStatus_content()));
        }
        ...
        holders[type].attention.setBackgroundResource(R.drawable.icon_delete);
        holders[type].attention.setText("");
        holders[type].attention.setMinEms(0);
...
//				.getUser_icon());
    ...
    class OnItemViewClickListener implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            switch (v.getId()) {
                ...
                case R.id.dynamic_type_other_picture_attention_btn:
                    showConfirmDialog(mContext, "确认删除？ 删除后不可恢复", false, holder.getPosition());
                    break;
                ...
                case R.id.dynamic_type_other_picture_card_ll:
                    ...
                    break;
                case R.id.dynamic_type_just_text_url_tv:
        ...
        public void showConfirmDialog(final Context context,
                                      CharSequence msg, final boolean isMust, final int position) {
        ...
        private void infoDel(final int position) {
            RequestParams params = TXApplication.getParams();
            params.addBodyParameter("idStr", mList.get(position).getIdStr());
            String url = URLs.MYPHOTO_INFODEL;
